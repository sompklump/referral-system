...
<html>
	<head>
		...
		<title>Refferal System - Requiem</title>
	</head>
	<body>
		<div>
			<div class="pos-f-t">
...
<span class="navbar-toggler-icon"></span>
						...
						<span class="user-creds-username user-creds-text">
							<p><?= $user_creds->username . "#" . $user_creds->discriminator ?></p>
						</span>
						<font color="white"><span class="user-creds-text requiem-yellow">Level <?= $user_points ?><span style="color:white;">,</span></span><span class="user-creds-text requiem-yellow">XP 384</span></font>
					</div>
					<div class="nav-right">
						<nav class="navbar-expand-sm">
							<ul class="navbar-nav">
								...
								<a href="../login/?logout" class="navbar-brand requm-link">Logout <i class="fas fa-sign-out-alt"></i></a>
							</ul>
						</nav>
					</div>
				</nav>
			...
			&nbsp;
			<?php
	$sql = "SELECT * FROM invites WHERE user = '$disc_userid'";
					$result = mysqli_query($conn, $sql);
					if (mysqli_num_rows($result) <= 0) {
        ...
        <table>
          ...
          <tr>";
						...
						if (mysqli_num_rows($result) > 0) {
							while ($row = mysqli_fetch_array($result)) {
								$created_date = date("d/m/Y", strtotime($row['create_date']));
